...
```
                                    ┌─────────────────────────────────────────────────────────────────┐
                                    │                        CLIENT LAYER                             │
                                    │              (Web App / Mobile App / Trading Terminal)          │
                                    └─────────────────────────────┬───────────────────────────────────┘
                                                                  │
                                                                  │ HTTPS Request
...
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐   │
│  │  Rate Limiter   │  │   JWT Filter    │  │  Route Matcher  │  │   CORS Filter   │  │  Request Logger │   │
│  │  (Redis-based)  │  │  (Validation)   │  │   (Routing)     │  │   (Security)    │  │  (Observability)│   │
│  └────────┬────────┘  └────────┬────────┘  └────────┬────────┘  └────────┬────────┘  └────────┬────────┘   │
│           └────────────────────┴────────────────────┴────────────────────┴─────────────────────┘            │
│                                                     │                                                       │
│                           ┌─────────────────────────┴─────────────────────────┐                             │
...
└─────────────────────────────────────────────────────┼───────────────────────────────────────────────────────┘
    ...
    │       (Port: 8081)        │   │       (Port: 8082)        │   │   (Order, Wallet, etc.)   │
    │                           │   │                           │   │                           │
    │  • Login / Register       │   │  • User Profile CRUD      │   │  • Protected Endpoints    │
    │  • JWT Generation         │◄──┤  • KYC Management         │   │  • JWT Validation         │
    │  • Token Refresh          │   │  • Preferences            │   │  • Role-based Access      │
    │  • 2FA / MFA              │   │  • Account Management     │   │                           │
    │  • Password Reset         │   │                           │   │                           │
    │  • Session Management     │   │                           │   │                           │
    └─────────────┬─────────────┘   └─────────────┬─────────────┘   └───────────────────────────┘
                  │                               │
...
│  (6379)   │             │  (5432)   │                   │  (5432)   │
│           │             │           │                   │           │
│ • Sessions│             │ • Auth DB │                   │ • User DB │
│ • Tokens  │             │ • Creds   │                   │ • Profile │
│ • Cache   │             │ • Roles   │                   │ • KYC     │
└───────────┘             └───────────┘                   └───────────┘
```
...
└─────────────────────────────────────────────────────────────────────────────────────────────────────┘
          ...
          │     │              │     │              │     │              │               │
          │     │ • All Access │     │ • Trade      │     │ • View       │               │
          │     │ • User Mgmt  │     │ • Withdraw   │     │ • Deposit    │               │
          │     │ • System Cfg │     │ • View       │     │ • Profile    │               │
          │     │ • Audit Logs │     │ • Deposit    │     │              │               │
          │     └──────────────┘     └──────────────┘     └──────────────┘               │
          │            │                    │                    │                        │
          │            ▼                    ▼                    ▼                        │
          ...
          │     │  /api/orders/**        → @PreAuthorize("hasRole('TRADER')")     │      │
          │     │  /api/portfolio/**     → @PreAuthorize("hasRole('USER')")       │      │
          │     │  /api/users/profile    → @PreAuthorize("isAuthenticated()")     │      │
...
│                                      │          │                                      │
│  ┌────────────────────────────────┐  │          │  ┌────────────────────────────────┐  │
│  │       AuthController           │  │   REST   │  │       UserController           │  │
│  │  • POST /auth/login            │◄─┼──────────┼─►│  • GET  /users/{id}            │  │
│  │  • POST /auth/register         │  │   API    │  │  • POST /users                 │  │
│  │  • POST /auth/refresh          │  │  (Feign) │  │  • PUT  /users/{id}            │  │
│  │  • POST /auth/logout           │  │          │  │  • GET  /users/profile         │  │
│  │  • POST /auth/verify-2fa       │  │          │  │  • POST /users/kyc             │  │
...
│  │  • validateCredentials()       │  │          │  │  • createUser()                │  │
│  │  • generateTokens()            │  │          │  │  • getUserById()               │  │
│  │  • refreshTokens()             │  │          │  │  • updateProfile()             │  │
│  │  • validateToken()             │  │          │  │  • verifyKYC()                 │  │
│  │  • revokeToken()               │  │          │  │  • getUserRoles()              │  │
│  └────────────────────────────────┘  │          │  └────────────────────────────────┘  │
│               │                      │          │               │                      │
│               ▼                      │          │               ▼                      │
│  ┌────────────────────────────────┐  │          │  ┌────────────────────────────────┐  │
│  │       Repository Layer         │  │          │  │       Repository Layer         │  │
│  │  • AuthCredentialRepository    │  │          │  │  • UserRepository              │  │
│  │  • RoleRepository              │  │          │  │  • UserProfileRepository       │  │
│  │  • RefreshTokenRepository      │  │          │  │  • KYCRepository               │  │
│  └────────────────────────────────┘  │          │  └────────────────────────────────┘  │
│               │                      │          │               │                      │
│               ▼                      │          │               ▼                      │
│  ┌────────────────────────────────┐  │          │  ┌────────────────────────────────┐  │
...
│  │   │ • roles                 │  │  │          │  │   │ • user_profiles         │  │  │
│  │   │ • user_roles            │  │  │          │  │   │ • user_preferences      │  │  │
│  │   │ • password_reset_tokens │  │  │          │  │   │ • kyc_documents         │  │  │
│  │   └─────────────────────────┘  │  │          │  │   │ • user_addresses        │  │  │
│  │   ┌─────────────────────────┐  │  │          │  │   └─────────────────────────┘  │  │
│  │   │ Redis:                  │  │  │          │  │                                │  │
│  │   │ • refresh_tokens:{id}   │  │  │          │  │                                │  │
